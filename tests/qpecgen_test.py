...
import core.matrix_handling as mh
...
param_inst.update({
    'p':param_inst['m'],               # Number of the second level constraints
                                       # for AVI-MPEC.
...
class test_helpers(unittest.TestCase):
    ...
    def test_min_max_diag(self):
        for M in self.Mlist:
            self.assertEqual(hp.mindiag(M), min(M[i, i] for i in range(min(M.shape))))
            self.assertEqual(hp.maxdiag(M), max(M[i, i] for i in range(min(M.shape))))
...
class test_base(unittest.TestCase):
    def setUp(self):
        ...
        for i in range(K):
            ...
            self.Plist.append(Qpecgen100('test_100_c_{0}'.format(i), param_inst))
            self.Plist.append(Qpecgen200('test_200_c_{0}'.format(i), param_inst))
            self.Plist.append(Qpecgen201('test_201_c_{0}'.format(i), param_inst))
            param_inst.update({'convex_f': False})
            self.Plist.append(Qpecgen100('test_100_nc_{0}'.format(i), param_inst))
            self.Plist.append(Qpecgen200('test_200_nc_{0}'.format(i), param_inst))
            self.Plist.append(Qpecgen201('test_201_nc_{0}'.format(i), param_inst))
...
#            print investigate_200_info(P)
            certificates_dict = Q.get_violations(Q.details['gensol'], silence=True)
    ...
    def test_suitable_A(self):
        for P in self.Plist:
            ...
            if not P.param['yinfirstlevel']:
                ypart = P.A[:, P.param['n']:]
                np.testing.assert_equal(ypart, hp.zeros(P.param['l'], P.param['m']))
...
#        info = P.info
#        param = P.param
#        assert info['l'] == info['l_deg'] + info['l_nonactive'] + info['l_active']
#        assert info['m'] == info['ms'] + info['md']
#        assert param['second_deg'] == info['ms_deg'] +
#                  info['md_upp_deg'] + info['md_low_deg']
